...
export function Products(){
    ...
    function getProductsByColour(products,colour){
        const colouredProducts= [...products].filter((product)=>colour.length>0 ? colour.includes(product.colour) : product);
        console.log("Coloured products::",colouredProducts);
        return colouredProducts;
    }
    const colouredProducts= getProductsByColour(searchedProduct, filterState.colour);
    ...
    <>
        <Navigation isSearchbarVisible="true" />
            ...
            {isLoading? <Loader /> : 
            <>
                {(colouredProducts) && (colouredProducts.length>0) &&
                    colouredProducts.map((product)=>
                        (
